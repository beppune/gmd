...
data class OrderDetailsPresentation(
...
class OrderDetailsForm(
    ...
    val binder = Binder(OrderPresentation::class.java)
    ...
    private fun gui() {
        typeField.setItemLabelGenerator {
            when(it) {
                Order.Type.INBOUND -> "CARICO"
                Order.Type.OUTBOUND -> "SCARICO"
            }
        }
        typeField.setItems(Order.Type.values().toList())

        subjectField.setItemLabelGenerator {
            when(it) {
        ...
        subjectField.setItems(Order.Subject.values().toList())

        dcField.setItemLabelGenerator { "${it.short} - ${it.fullName}" }
        dcField.setItems(dcs.findAll(true))

        supplierField.setItems(sups.findAll())

/* =====================================================================
   ...
   ESCLUDE:
     - Anagrafiche (dcs/locations/items/suppliers/operators/permissions)
   ...
   NOTE:
     - ref_value NON NULL: REP (JSON) -> magazzino.utenti -> gmd.operators -> user_oldid -> 'UNKNOWN'
     - orders_lines: lo schema consigliato prevede PK su id AUTO_INCREMENT (evita dup su PK composita)
...
/* ---------------------------------------------------------------------
   0) DDL NOTE (orders_lines)
   Se la tua gmd.orders_lines ha ancora PK composita (ownedby,datacenter,item,pos,amount)
   puoi incorrere in duplicati con amount uguale (es. 1) e più seriali.
   Schema consigliato:
     - aggiungi colonna id BIGINT AUTO_INCREMENT PRIMARY KEY
     - UNIQUE (ownedby,sn) e UNIQUE (ownedby,pt)
   --------------------------------------------------------------------- */
...
SELECT
  o.id AS ownedby,
  TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(o.remarks, 'DOC=', -1), ' |', 1)) AS doc_interno
FROM gmd.orders o
...
-- Audit righe senza ownedby
INSERT INTO gmd.orders_lines_audit(issue_type, doc_interno, src_id, details)
SELECT 'ORDER_NOT_FOUND', odm.doc_interno, s.src_id, 'Nessun ordine trovato per DOC=<NumeroDocInterno>'
FROM gmd.orders_lines_stage s
LEFT JOIN gmd.order_doc_map odm ON odm.ownedby = s.ownedby
